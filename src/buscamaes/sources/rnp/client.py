...
class RNPClient:
    ...
    async def _login(self) -> None:
        ...
        has_auth_cookie = any(k.startswith("TSf") for k in self._session.cookies.keys())
        if not has_auth_cookie:
            if "Datos incorrectos" in login_post.text:
                raise RuntimeError("invalid credentials")
            logger.warning("Login failed — no TSf* cookie. Present: %s", list(self._session.cookies.keys()))
